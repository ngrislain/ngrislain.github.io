...
The continuous SSM is defined by the ODE $`\dot{h}(t) = A(t) h(t) + B(t) x(t)`, which must be _discretized_ to get a usable recurrence. Mamba-1 and Mamba-2 used a heuristic approximation that the authors now formalize as "exponential-Euler" — an exponential integrator combined with Euler's rule for the state-input integral. The key insight is that Euler's rule is only first-order accurate. Replacing it with a _trapezoidal rule_ — a data-dependent convex combination of both interval endpoints — yields a second-order method:
