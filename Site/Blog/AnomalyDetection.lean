...
This is the UEBA critique, in one panel. 4.5% of characters past 3 nats is one flagged item in twenty-two, on a page where nothing whatsoever is wrong. Every one of them is a false positive, and `web search` and `virtual` are as loud as most of what the model will ever say. Put that same rate on a fleet of users and hosts, each with its own fitted histogram, and it is thousands of alerts a day to be looked at and dismissed. The noise is not a tuning failure that better thresholds would fix. It is what a short-context frequency table does when it meets ordinary variation it has not seen enough of. Qwen's floor on the same text is 0.3%, one in three hundred and thirty, with nothing tuned and nothing fitted.
...
Threshold each character on its own and count, and the n-gram marks far more of the insertion: 95% of the Basque characters pass 3 nats against Qwen's 3%. But counting flagged characters is the wrong question to ask of a signal shaped like Qwen's. It fires at both edges, and two edges are a region. The entry spike sits on the space before `Euskara`, the exit spike on the newline that closes the sentence, and what lies between them is the insertion, delimited exactly. The n-gram's plateau is denser but blunter at the ends: nineteen characters after the splice is over it is still above its own average, reading 6.4 nats on the `H` of `High-profile`.

So the fair version of this is narrower. What a sustained signal buys is redundancy. The n-gram's evidence is spread over two hundred characters and no single one of them carries it. Qwen's sits in two tokens. Lose those, to a context window that truncates or a batch boundary that happens to fall there, and the region goes unmarked even though the model would have said so, loudly, at the moment it changed. Redundancy is worth something in a pipeline that drops data.
...
On [public benchmarks](https://www.datadoghq.com/blog/ai/ai-security-detection-pipeline/) it reaches F1 1.000 on Thunderbird, 0.999 on BGL and 0.994 on UNSW-NB15, above the previous best, and trails slightly on LANL Cyber1 (AUC 0.987 against 0.990), HDFS (F1 0.941 against 0.980) and CERT Insider (AUC 0.879 against 0.910). The individual numbers matter less than the fact that one model with one recipe gets there on all of them. That is the property a per-entity histogram can never have.

# Why this is a real break

Detection engineering has spent twenty years encoding what somebody already knew to look for. There is a ladder in that, and each rung asks for less prior knowledge than the one below it.

A signature names the artifact: this file hash, this domain, this request string. It catches the attack it was written for and nothing adjacent to it. A correlation rule gives up the artifact and names the shape instead: twenty failed logins, then a success, then a new admin role, all inside five minutes. That survives a change of payload or tooling, but somebody still had to picture the sequence in advance and write it down. A fitted behavioral baseline gives up the picturing too. It learns what one entity normally does and flags departures from it, which is a real step, and still a narrow one: it knows a single entity, along the features you thought to model, and only once it has converged.

*A pretrained sequence model encodes what usually happens.* Not for this user, not for this feature, but _in general_. It can be surprised by something nobody wrote a rule for, in a place nobody thought to put a histogram, on an entity that has existed for four minutes. And the score it produces is a plain likelihood, which composes and thresholds like any other number.
...
That is the part worth sitting with. The attention on generative AI goes to what it writes, but the same pretraining that makes a model fluent is what makes it a detector, and *it arrives already knowing what normal looks like, for every entity, on day one*. No fitting window, no per-user histogram, no thirty days of waiting. At the scale security data actually arrives at, that is not a better version of the old thing. It removes the step the old thing was built around.
