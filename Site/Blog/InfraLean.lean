...
Scope follows from the ledger, and there are only two cases. A resource with a row is mine: delete its line and it gets destroyed. A resource without one is not mine and is never touched, whatever it is called, whoever created it, whatever tags it carries. Nothing else is consulted.

That the ledger is a list of what to *include* rather than a list of what to *avoid* is the whole safety argument, and the two fail in opposite directions. A missing row means something keeps running that nobody tracks, which costs money until someone notices. A missing entry on an avoid-list would mean deleting a stranger's database, which costs rather more, and only once. So the tool claims nothing it cannot point at a row for. Run it against an account full of somebody else's work and it proposes no deletions at all.

The price is what happens when the ledger goes. Wipe `.infra/` and the rows go with it, and they are the only record those resources ever had: the declaration no longer names the ones whose lines you deleted, and a plan only ever reads back what the declaration names, so it will not find them either. They keep running and nothing in the tool can name them again. Nor can I write a command to sweep the account and rebuild the rows, because there is nothing on the resources to sweep for. That is the gap the repo sketches a fix for: a tag written onto everything `infra` creates, so ownership is a property of the resource rather than of a file on my laptop, an account the tool refuses to operate outside of, and a list of pre-existing resources to leave alone whatever the tag says. Only the account check is built (and only if you name the account). Nothing writes the tag yet, so the ledger is still the only boundary there is.

None of that is what types are for either. Ownership is about what happened on some past machine, not about what is well-formed on this one. My favourite failure from the live runs:
