...
A place gets the same treatment, one level up. A locality is a place named before any cloud names it, and each cloud maps it to its own code or to nothing. `#guard` is Lean's way of writing that down as a claim the compiler checks then and there: it evaluates the proposition after it, and if it is false, the file fails to compile with that proposition quoted back at you; if it is true, nothing is printed and compilation moves on. It is a unit test with no test runner, no separate `go test` or `pytest`, just a line in the same file, checked the moment the file is checked:
...
`Repr`, `ToJson` and `FromJson` are the typeclasses that let a value be printed for debugging, serialized, and read back. None is derived here, on purpose: there is no automatic way to print, log, or serialize a `SecretSource`. The one exception is a `Repr` I wrote by hand myself, and it always prints `<redacted>`, so even a composed value cannot reach a stray trace. A plan is then checked for whether every secret's source is sound, and it rejects the laundered version too:
...
The first guard is the easy case: `"hunter2"` sits right there as a `SecretSource.composed` value, so a check that just pattern-matches the constructor catches it. The second is the one worth having. There `composed` is not the constructor, it is the smart constructor that builds an `Expr`, and `.lit "hunter2"` reaches it wrapped in that expression machinery rather than sitting bare, the way a real attempt to sneak a literal past a naive check would try to. `sourceIsSound` does not pattern-match syntax, though, it walks the expression looking for a `fromEnv` anywhere underneath, and there is none, so the wrapping buys nothing and the guard still rejects it.
...
The two holes in that string are two arrows in the graph.

Secrets are the obvious case, but they are not the only one: any field can hold such a recipe, and any post-apply value can go in one. `secretValueOf` and `endpointOf` are simply the two projections common enough to have names in the library, and each is one line over the general form, which is `observedOf` and a field:

```
def privateIpOf {p : ProviderId} (key : K p .awsInstance) : Expr K String :=
  (observedOf key).map (·.privateIp)
```

An instance's private address does not exist until the instance launches, so a peer list built out of two of them is the connection string again, one kind further out:

```
fleet cluster in paris where
  resource aws securityGroup "web" as web
    { description := "http and https, ssh from nowhere" }

  resource aws awsInstance "app-1" as app1
    { imageId       := "latest"
    , instanceType  := InstanceType.of .t3 .nano
    , securityGroup := web }

  resource aws awsInstance "app-2" as app2
    { imageId       := "latest"
    , instanceType  := InstanceType.of .t3 .nano
    , securityGroup := web }

  resource aws secrets "cluster-peers"
    { valueFrom := composed expr!"{privateIpOf app1},{privateIpOf app2}" }
```

Three kinds, two ways of referring to another resource, and one diamond. `securityGroup := web` is a reference in a field: the group has to exist before either instance, and the type says so. The two holes in the last string are references inside a value: each instance has to exist, and have launched, before the peer list can be written. Nothing distinguishes them to the scheduler, because both report the same edges:

![the web security group feeds both app-1 and app-2, which both feed the cluster-peers secret](static/blog/infra-lean/dag.svg)

Four resources, four creates, one apply. The order comes from those arrows: nothing in the file says the group goes first, or that both instances are running before the peer list is written. There is no `depends_on` in the language, because a reference _is_ the dependency, so the graph cannot disagree with the code. Creation is Kahn's algorithm over the edges the references report, and deletion is the same graph transposed, so teardown is the reverse rather than a separate guess. That last part matters here: EC2 refuses to delete a security group an instance still references, and the reversed diamond gets the instances gone first without anyone saying so.
...
Now write the same fleet in HCL. It has to give up on every one of the five:

:::sideBySide "the fleet, three of the five" "resource secrets \"db-base\" as base\n  { valueFrom := fromEnv \"DB_PASSWORD\" }\n\nresource secrets \"app-a\" as a\n  { valueFrom := composed\n      expr!\"a:{secretValueOf base}\" }\n\nresource secrets \"app-tail\"\n  { valueFrom := composed\n      expr!\"t:{secretValueOf sink}\" }" "main.tf (HCL)" "resource \"aws_secretsmanager_secret\" \"db-base\" {\n  name   = \"db-base\"\n  # TODO value: a secret's value is never in the\n  # declaration; wire it up in Terraform yourself\n  region = \"eu-west-3\"\n}\n\nresource \"aws_secretsmanager_secret\" \"app-a\" {\n  name   = \"app-a\"\n  # TODO value: a secret's value is never in the\n  # declaration; wire it up in Terraform yourself\n  region = \"eu-west-3\"\n}\n\nresource \"aws_secretsmanager_secret\" \"app-tail\" {\n  name   = \"app-tail\"\n  # TODO value: a secret's value is never in the\n  # declaration; wire it up in Terraform yourself\n  region = \"eu-west-3\"\n}"
:::

Those `# TODO` lines are HCL refusing to guess, and I think they are the fairest picture of the gap. The names and the regions translate. The graph does not, because in HCL a secret's value is a second resource wired up by hand, and composing one from three others is yours to assemble and yours to order.
...
*One table does three jobs.* The table of places maps each place to each cloud's own code. Autocomplete lists the places from it, the compile-time check reads it, and the assertions pin its entries: three consumers reading one source, not three copies of the same list that could quietly diverge. In HCL the equivalent knowledge, which region codes a provider accepts, lives in the provider's Go source, in its documentation, and in your head, and those three disagree often enough to be its own class of bug report.
...
What the live run in CI actually exercises, and what it leaves out:
